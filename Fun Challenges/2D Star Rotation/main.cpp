...
int main() {
  std::vector<point3d> star;
  int numPoints{5};
  float outerRadius{10.0f}, innerRadius{5.0f};
  for (int i{0}; i < numPoints * 2; ++i) {
    float angle{static_cast<float>(i) * M_PI / numPoints};
  ...
  while (true) {
    std::vector<point3d> rotatedStar{star};
    for (auto &p : rotatedStar) {
      rotateX(p, angleX);
      rotateY(p, angleY);
      rotateZ(p, angleZ);
    }
    std::system("CLS");
    renderer(rotatedStar);
    angleX += 1.0f;
    angleY += 1.0f;
    angleZ += 1.0f;
    usleep(50000);
